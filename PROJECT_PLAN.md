...
> 基于 LangGraph + Chroma 的开源多智能体网文翻译引擎
...
网文出海面临两个核心瓶颈：**翻译成本高**（人工 $30-60/章，一本千章小说需 $30,000+）和**文化壁垒深**（中式表达在海外读者中理解率低，直译不可读）。本项目用 Agent 技术将翻译成本降低 99%+，同时通过文化适配让译文读起来像 Native English 网文，而非机器翻译。
...
**设计要点**：SQLite 是零运维的嵌入式数据库，checkpoint 写入是原子操作，不会出现半章脏数据。glossary_snapshot 的存在意味着即使 Chroma 文件损坏，精确层术语表也不会丢。
...
**关于成本估算**：实际部署前应去 DeepSeek 官网确认最新 pricing。但不管具体定价如何，Agent 翻译相比人工翻译 $30-60/章，成本降低两个数量级这个结论是稳定的。
...
这个演进路线体现了从 MVP 快速验证到规模化生产的一步步迭代思路。
